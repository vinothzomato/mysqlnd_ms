...
if ($error = mst_create_config("test_mysqlnd_ms_pick_user_multi_multi_master.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
...
<?php
	...
	function pick_servers($connected_host, $query, $masters, $slaves, $last_used_connection, $in_transaction) {
		...
		return array(array(1, 0), array(0));
	}
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_pick_user_multi_multi_master.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_pick_user_multi_multi_master.ini'.\n");
?>
--EXPECTF--
pick_server('myapp', '/*ms=master*//*2*/SELECT 1 FROM DUAL, '', 2, 1)
array(1) {
  [1]=>
  string(1) "1"
}
