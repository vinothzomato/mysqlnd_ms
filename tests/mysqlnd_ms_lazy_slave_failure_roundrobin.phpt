...
if ($error = create_config("test_mysqlnd_ms_lazy_slave_failure_roundrobin.ini", $settings))
	die(sprintf("SKIP %d\n", $error));
?>
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.ini_file=test_mysqlnd_ms_lazy_slave_failure_roundrobin.ini
--FILE--
<?php
	require_once("connect.inc");
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_lazy_slave_failure_roundrobin.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_lazy_slave_failure_roundrobin.ini'.\n");
?>
--EXPECTF--
