--TEST--
table filter: rule with empty pattern (random once)
--SKIPIF--
<?php
...
if ($error = mst_create_config("test_mysqlnd_ms_table_rule_empty_pattern_cont_ro.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
?>
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.ini_file=test_mysqlnd_ms_table_rule_empty_pattern_cont_ro.ini
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_table_rule_empty_pattern_cont_ro.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_table_rule_empty_pattern_cont_ro.ini'.\n");
?>
--EXPECTF--
