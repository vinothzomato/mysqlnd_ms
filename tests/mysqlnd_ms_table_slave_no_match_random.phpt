--TEST--
table filter: no slave for SELECT, use master - pick = random
--SKIPIF--
<?php
...
if ($error = mst_create_config("test_mysqlnd_ms_slave_no_match_random.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
?>
--INI--
mysqlnd_ms.enable=1
...
<?php
	...
	if (!unlink("test_mysqlnd_ms_slave_no_match_random.ini"))
