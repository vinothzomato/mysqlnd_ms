--TEST--
trx_stickiness=on, failover, random
--SKIPIF--
<?php
...
$settings = array(
	"myapp" => array(
		...
		'slave' => array($emulated_slave_host, "unknown:7033", $emulated_slave_host),
		'trx_stickiness' => 'on',
		'pick' => array("random"),
		'lazy_connections' => 1,
...
if ($error = mst_create_config("test_mysqlnd_ms_trx_stickiness_slave_failover_random.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
...
mysqlnd_ms.config_file=test_mysqlnd_ms_trx_stickiness_slave_failover_random.ini
--FILE--
<?php
	require_once("connect.inc");
	...
	/* trying to hit the unavailable slave */
	$i = 0;
	$servers = array();
	do {
		$last = NULL;
	...
	printf("Number of server contacted %d\n", count($servers));

	print "done!";
?>
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_trx_stickiness_slave_failover_random.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_trx_stickiness_slave_failover_random.ini'.\n");
?>
--EXPECTF--
