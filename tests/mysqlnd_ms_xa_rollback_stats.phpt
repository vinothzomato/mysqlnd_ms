...
if ($error = mst_create_config("test_mysqlnd_ms_xa_rollback_stats.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
?>
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.config_file=test_mysqlnd_ms_xa_rollback_stats.ini
...
<?php
	...
	if (!mysqlnd_ms_xa_begin($link, $xa_id) || !mysqlnd_ms_xa_rollback($link, $xa_id + 1)) {
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_xa_rollback_stats.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_xa_rollback_stats.ini'.\n");
?>
--EXPECTF--
[004] xa_begin: 0 -> 1
[005] xa_rollback_success: 0 -> 1

Warning: mysqlnd_ms_xa_rollback(): (mysqlnd_ms) The XA transaction id does not match the one of from XA begin in %s on line %d
[006] [2000] '(mysqlnd_ms) The XA transaction id does not match the one of from XA begin'
[007] xa_begin: 1 -> 2
[008] xa_rollback_success: 1 -> 2
