...
if ($error = mst_create_config("test_mysqlnd_ms_lazy_slave_failure_failover_loop_max_retries_rr.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
...
mysqlnd_ms.config_file=test_mysqlnd_ms_lazy_slave_failure_failover_loop_max_retries_rr.ini
...
<?php
	...
	compare_stats(3, $stats, $exp_stats, array(
		"lazy_connections_slave_failure", "lazy_connections_slave_success",
		));
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_lazy_slave_failure_failover_loop_max_retries_rr.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_lazy_slave_failure_failover_loop_max_retries_rr.ini'.\n");
?>
--EXPECTF--
no result 0,slave-%d 1,master-%d 2,slave-%d 3,no result 4,slave-%d 5,master-%d 6,slave-%d 7,no result 8,slave-%d 9,
master-%d 0,no result 1,master-%d 2,no result 3,master-%d 4,no result 5,master-%d 6,no result 7,master-%d 8,no result 9,
done!