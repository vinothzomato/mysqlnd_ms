...
if ($error = mst_create_config("test_mysqlnd_ms_table_parser15.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
...
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.config_file=test_mysqlnd_ms_table_parser15.ini
--FILE--
<?php
	require_once("connect.inc");
	...
	$sql = "SELECT DISTINCTROW id AS _id FROM test ORDER BY id ASC";
...
<?php
	...
	if (!unlink("test_mysqlnd_ms_table_parser15.ini"))
		printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_table_parser15.ini'.\n");

	if ($err = mst_mysqli_drop_test_table($emulated_slave_host_only, $user, $passwd, $db, $emulated_slave_port, $emulated_slave_socket))
		printf("[clean] %s\n", $err);
?>
--EXPECTF--
[001] Testing server support of 'SELECT DISTINCTROW id AS _id FROM test ORDER BY id ASC'
[006] _id = '1'
done!