...
if ($error = mst_create_config("test_mysqlnd_ms_table_parser25.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
...
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.config_file=test_mysqlnd_ms_table_parser25.ini
--FILE--
<?php
	require_once("connect.inc");
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_table_parser25.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_table_parser25.ini'.\n");
?>
--EXPECTF--
[001] Testing server support of 'SELECT \N AS _id FROM DUAL'
[006] _id = ''
done!