...
if ($error = mst_create_config("test_mysqlnd_ms_pick_ro_user_multi_no_master.ini", $settings))
	die(sprintf("SKIP %s\n", $error));

?>
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.config_file=test_mysqlnd_ms_pick_ro_user_multi_no_master.ini
--FILE--
<?php
	require_once("connect.inc");
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_pick_ro_user_multi_no_master.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_pick_ro_user_multi_no_master.ini'.\n");
?>
--EXPECTF--
