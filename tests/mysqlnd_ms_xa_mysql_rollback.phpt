...
if ($error = mst_create_config("test_mysqlnd_ms_xa_mysql_rollback.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
?>
--INI--
mysqlnd_ms.enable=1
...
<?php
	...
	/* Plain connect and rollback followed by begin */
...
<?php
	...
	if (!unlink("test_mysqlnd_ms_xa_mysql_rollback.ini")) {
		printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_xa_mysql_rollback.ini'.\n");
	}
