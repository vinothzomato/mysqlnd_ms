...
if ($error = mst_create_config("test_mysqlnd_ms_table_parser29.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
...
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.config_file=test_mysqlnd_ms_table_parser29.ini
--FILE--
<?php
	require_once("connect.inc");
	...
	$sql = "SELECT id OR id AS _id FROM test AS d";
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_table_parser29.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_table_parser29.ini'.\n");
?>
--EXPECTF--
