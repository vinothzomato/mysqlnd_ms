--TEST--
table filter: no master rule (random)
--SKIPIF--
<?php
...
if ($error = mst_create_config("test_mysqlnd_ms_table_rule_master_empty_random.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
?>
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.ini_file=test_mysqlnd_ms_table_rule_master_empty_random.ini
--FILE--
<?php
	require_once("connect.inc");
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_table_rule_master_empty_random.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_table_rule_master_empty_random.ini'.\n");
?>
--EXPECTF--
