...
if ($error = mst_create_config("test_mysqlnd_ms_pick_random_weight_list_length_error.ini", $settings))
	die(sprintf("SKIP %s\n", $error));

?>
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.config_file=test_mysqlnd_ms_pick_random_weight_list_length_error.ini
--FILE--
<?php
	require_once("connect.inc");
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_pick_random_weight_list_length_error.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_pick_random_weight_list_length_error.ini'.\n");
?>
--EXPECTF--
