--TEST--
Unreachable master, pick = random once, lazy = 0
--SKIPIF--
<?php
...
if ($error = mst_create_config("test_mysqlnd_ms_master_unreachable_random_once.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
?>
--INI--
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_master_unreachable_random_once.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_master_unreachable_random_once.ini'.\n");
?>
--EXPECTF--
