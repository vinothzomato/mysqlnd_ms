--TEST--
parser: SELECT SQL_CALC_FOUND_ROWS
--SKIPIF--
<?php
...
if ($error = create_config("test_mysqlnd_ms_table_parser16.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
?>
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.ini_file=test_mysqlnd_ms_table_parser16.ini
--FILE--
<?php
	require_once("connect.inc");
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_table_parser16.ini"))
...
[003 + 01] Query 'SELECT SQL_CALC_FOUND_ROWS id AS _id FROM test ORDER BY id ASC'
[003 + 02] Thread '%d'
[004] _id = '%d'
done!