...
if ($error = mst_create_config("test_mysqlnd_ms_offline_real_escape_change.ini", $settings))
	die(sprintf("SKIP %s\n", $error));

?>
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.config_file=test_mysqlnd_ms_offline_real_escape_change.ini
--FILE--
<?php
	require_once("connect.inc");
...
--CLEAN--
<?php
if (!unlink("test_mysqlnd_ms_offline_real_escape_change.ini"))
	printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_offline_real_escape_change.ini'.\n");
?>
--EXPECTF--
set_charset()
query()
