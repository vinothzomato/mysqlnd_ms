...
if ($error = mst_create_config("test_mysqlnd_ms_unknown_filter2.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
?>
--INI--
mysqlnd_ms.enable=1
...
?>
--EXPECTF--
Warning: mysqli_real_connect(): (HY000/2000): (mysqlnd_ms) Unknown filter 'a' . Stopping in %s on line %d
[001] [2000] (mysqlnd_ms) Unknown filter 'a' . Stopping
done!