...
if ($error = mst_create_config("test_mysqlnd_ms_trx_stickiness_slave_failover_rr.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
...
mysqlnd_ms.config_file=test_mysqlnd_ms_trx_stickiness_slave_failover_rr.ini
--FILE--
<?php
	require_once("connect.inc");
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_trx_stickiness_slave_failover_rr.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_trx_stickiness_slave_failover_rr.ini'.\n");
?>
--EXPECTF--
