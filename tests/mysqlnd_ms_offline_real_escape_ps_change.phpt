--TEST--
offline_server_charset + set_charset + ps
--SKIPIF--
<?php
...
if ($error = mst_create_config("test_mysqlnd_ms_offline_real_escape_ps_change.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
...
<?php
	...
	if (!($stmt = $link_ms->prepare("SELECT @@character_set_connection AS charset"))) {
		printf("[007] [%d/%s] '%s'\n", $link_ms->errno, $link_ms->sqlstate, $link_ms->error);
	}
...
--CLEAN--
<?php
if (!unlink("test_mysqlnd_ms_offline_real_escape_ps_change.ini"))
	printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_offline_real_escape_ps_change.ini'.\n");
?>
--EXPECTF--
set_charset()
query()
