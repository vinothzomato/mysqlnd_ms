--TEST--
force_server_charset + set_charset
--SKIPIF--
<?php
...
$settings = array(
	"myapp" => array(
		...
		'force_server_charset' => $row['charset'],
		'lazy_connections' => 1,
	),
);
if ($error = mst_create_config("test_mysqlnd_ms_force_charset_real_escape_change_later.ini", $settings))
	die(sprintf("SKIP %s\n", $error));

?>
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.config_file=test_mysqlnd_ms_force_charset_real_escape_change_later.ini
--FILE--
<?php
	require_once("connect.inc");
	...
	if ($no_ms !== $ms) {
		printf("[003] Encoded strings differ for charset '%s', MS = '%s', no MS = '%s'\n", $row['charset'], $ms, $no_ms);
	...
	printf("query()\n");
	/* Connection gets opened, CS tricks shall be tested for */
	$res = mst_mysqli_query(6, $link_ms, "SELECT @@character_set_connection AS charset");
	$row_ms = $res->fetch_assoc();
	if ($row['charset'] != $row_ms['charset']) {
		printf("[007] Expecting charset '%s' but '%s' is reported", $row['charset'], $row_ms['charset']);
	}
	...
	/* Connection is established, what about CS - limit? */
	printf("query()\n");
	$res = mst_mysqli_query(9, $link_ms, "SELECT @@character_set_connection AS charset");
	$row_ms = $res->fetch_assoc();
	if ($other_charset != $row_ms['charset']) {
		printf("[010] Expecting charset '%s' but '%s' is reported", $other_charset, $row_ms['charset']);
	}
	...
	printf("query()\n");
	$res = mst_mysqli_query(12, $link_ms, "SELECT @@character_set_connection AS charset");
	$row_ms = $res->fetch_assoc();
	if ($row['charset'] != $row_ms['charset']) {
		printf("[013] Expecting charset '%s' but '%s' is reported", $row['charset'], $row_ms['charset']);
	}
...
--CLEAN--
<?php
if (!unlink("test_mysqlnd_ms_force_charset_real_escape_change_later.ini"))
	printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_force_charset_real_escape_change_later.ini'.\n");
?>
--EXPECTF--
query()
set_charset()
