...
if ($error = mst_create_config("test_mysqlnd_ms_qc_failed_slave.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
...
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.config_file=test_mysqlnd_ms_qc_failed_slave.ini
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_qc_failed_slave.ini"))
