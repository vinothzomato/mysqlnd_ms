...
if ($error = mst_create_config("test_mysqlnd_ms_table_parser21.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
...
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.config_file=test_mysqlnd_ms_table_parser21.ini
--FILE--
<?php
	require_once("connect.inc");
	...
	if (mst_mysqli_server_supports_query(1, $sql, $emulated_slave_host_only, $user, $passwd, $db, $emulated_slave_port, $emulated_slave_socket)) {
	  ...
	  /* fake result */
	  printf("[006] _id = -320326809000000'\n");
	}
