--TEST--
Random, user multi, no master, non lazy
--SKIPIF--
<?php
...
if ($error = mst_create_config("test_mysqlnd_ms_pick_random_user_multi_no_master_non_lazy.ini", $settings))
	die(sprintf("SKIP %s\n", $error));

?>
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.config_file=test_mysqlnd_ms_pick_random_user_multi_no_master_non_lazy.ini
--FILE--
<?php
	require_once("connect.inc");
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_pick_random_user_multi_no_master_non_lazy.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_pick_random_user_multi_no_master_non_lazy.ini'.\n");
?>
--EXPECTF--
