...
if ($error = mst_create_config("test_mysqlnd_ms_trx_stickiness_no_slave_failover_random.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
...
mysqlnd_ms.config_file=test_mysqlnd_ms_trx_stickiness_no_slave_failover_random.ini
--FILE--
<?php
	require_once("connect.inc");
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_trx_stickiness_no_slave_failover_random.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_trx_stickiness_no_slave_failover_random.ini'.\n");
?>
--EXPECTF--
