...
if ($error = mst_create_config("test_mysqlnd_ms_gtid_invalid_on_commit.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
?>
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.ini_file=test_mysqlnd_ms_gtid_invalid_on_commit.ini
--FILE--
<?php
	require_once("connect.inc");
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_gtid_invalid_on_commit.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_gtid_invalid_on_commit.ini'.\n");
?>
--EXPECTF--
Catchable fatal error: mysqli_real_connect(): (mysqlnd_ms) 'on_commit' from 'global_transaction_id_injection' must be a string in %s on line %d