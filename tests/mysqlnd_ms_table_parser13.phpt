...
if ($error = create_config("test_mysqlnd_ms_table_parser13.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
?>
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.ini_file=test_mysqlnd_ms_table_parser13.ini
--FILE--
<?php
	require_once("connect.inc");
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_table_parser13.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_table_parser13.ini'.\n");
?>
--EXPECTF--
[001] Testing server support of 'SELECT SQL_CACHE id AS _id FROM test ORDER BY id ASC'
[003 + 01] Query 'SELECT SQL_CACHE id AS _id FROM test ORDER BY id ASC'
[003 + 02] Thread '%d'
[004] _id = '%d'
done!