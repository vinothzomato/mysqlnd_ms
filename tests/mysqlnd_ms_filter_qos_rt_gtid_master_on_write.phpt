--TEST--
Filter QOS, master_on_write, GTID
--SKIPIF--
<?php
...
if ($error = mst_create_config("test_mysqlnd_ms_filter_qos_master_on_write.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
...
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.config_file=test_mysqlnd_ms_filter_qos_master_on_write.ini
--FILE--
<?php
	require_once("connect.inc");
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_filter_qos_master_on_write.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_filter_qos_master_on_write.ini'.\n");
?>
--EXPECTF--
Server: slave-%d
Server: master-%d
