--TEST--
table filter basics: undefined master, non lazy
--SKIPIF--
<?php
...
if ($error = mst_create_config("test_mysqlnd_ms_table_undefined_master_in_rule.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
?>
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.ini_file=test_mysqlnd_ms_table_undefined_master_in_rule.ini
--FILE--
<?php
	require_once("connect.inc");
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_table_undefined_master_in_rule.ini"))
