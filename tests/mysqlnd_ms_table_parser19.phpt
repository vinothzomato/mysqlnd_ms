...
if ($error = mst_create_config("test_mysqlnd_ms_table_parser19.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
...
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.config_file=test_mysqlnd_ms_table_parser19.ini
--FILE--
<?php
	require_once("connect.inc");
	...
	if (mst_mysqli_server_supports_query(1, $sql, $emulated_slave_host_only, $user, $passwd, $db, $emulated_slave_port, $emulated_slave_socket)) {
		...
		/* fake result */
		printf("[006] _id = 'some text'\n");
	}

	$sql = "SELECT n'some text' AS _id";
	if (mst_mysqli_server_supports_query(9, $sql, $emulated_slave_host_only, $user, $passwd, $db, $emulated_slave_port, $emulated_slave_socket)) {
...
<?php
	if (!unlink("test_mysqlnd_ms_table_parser19.ini"))
		printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_table_parser19.ini'.\n");
?>
--EXPECTF--
[001] Testing server support of 'SELECT N'some text' AS _id'
[006] _id = 'some text'
[009] Testing server support of 'SELECT n'some text' AS _id'
[012] _id = 'some text'
done!