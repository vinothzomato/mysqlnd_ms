...
if ($error = mst_create_config("test_mysqlnd_ms_force_charset_real_escape_change_back.ini", $settings))
	die(sprintf("SKIP %s\n", $error));

?>
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.config_file=test_mysqlnd_ms_force_charset_real_escape_change_back.ini
--FILE--
<?php
	require_once("connect.inc");
...
--CLEAN--
<?php
if (!unlink("test_mysqlnd_ms_force_charset_real_escape_change_back.ini"))
	printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_force_charset_real_escape_change_back.ini'.\n");
?>
--EXPECTF--
set_charset()
Bail and stop - this is *force* charset. Why is force = A; connect(); set_charset(B); real_escape(): set_charset(A); query() allowed? This sequence gives us a string with "faulty" CS.