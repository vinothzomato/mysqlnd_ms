--TEST--
pick = user, callback = public method
--SKIPIF--
<?php
...
if ($error = mst_create_config("test_mysqlnd_ms_pick_user_class_public.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
?>
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.ini_file=test_mysqlnd_ms_pick_user_class_public.ini
--FILE--
<?php
	require_once("connect.inc");
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_pick_user_class_public.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_pick_user_class_public.ini'.\n");
?>
--XFAIL--
Syntax not supported. Was supported with mysqlnd_ms_set_user_pick_server()
--EXPECTF--
picker->server('myapp', 'SELECT 1 FROM DUAL')
[002 + 01] [0] ''
picker->server('myapp', 'SET @my_role='master'')
[003 + 01] [0] ''
done!