--TEST--
table filter: no master for DROP, pick = random
--SKIPIF--
<?php
...
if ($error = mst_create_config("test_mysqlnd_ms_table_master_no_match_random.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
?>
--INI--
mysqlnd_ms.enable=1
...
<?php
	...
	if (!unlink("test_mysqlnd_ms_table_master_no_match_random.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_table_master_no_match_random.ini'.\n");
?>
--EXPECTF--
