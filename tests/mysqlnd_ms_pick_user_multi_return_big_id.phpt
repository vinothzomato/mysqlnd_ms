...
if ($error = mst_create_config("test_mysqlnd_ms_pick_user_multi_return_bid_id.ini", $settings))
	die(sprintf("SKIP %s\n", $error));

?>
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.ini_file=test_mysqlnd_ms_pick_user_multi_return_bid_id.ini
--FILE--
<?php
	require_once("connect.inc");
	...
	function pick_servers($connected_host, $query, $masters, $slaves, $last_used_connection, $in_transaction) {
		...
		return array(array(3), array("foo"));
	}
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_pick_user_multi_return_bid_id.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_pick_user_multi_return_bid_id.ini'.\n");
?>
--EXPECTF--
