...
if ($error = mst_create_config("test_mysqlnd_ms_table_parser24.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
...
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.config_file=test_mysqlnd_ms_table_parser24.ini
--FILE--
<?php
	require_once("connect.inc");
	require_once("util.inc");

	$sql = "SELECT b'1' + 0 AS _id FROM DUAL";
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_table_parser24.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_table_parser24.ini'.\n");
?>
--EXPECTF--
[001] Testing server support of 'SELECT b'1' + 0 AS _id FROM DUAL'
[006] _id = '1'
done!