	...
	*which_server = mysqlnd_ms_query_is_select(query, query_len, &forced TSRMLS_CC);
	...
	switch (*which_server) {
		case USE_SLAVE:
		{
			...
			/* LOCK on context ??? */
			switch (zend_hash_find(&filter->slave_context, fprint.c, fprint.len /*\0 counted*/, (void **) &context_pos)) {
				case SUCCESS:
...
restart_slave:
					if (restart_count < zend_llist_count(l)) {
						...
						if (connection) {
							...
							zend_hash_update(&filter->slave_context, fprint.c, fprint.len /*\0 counted*/, &connection, sizeof(MYSQLND *), NULL);
							smart_str_free(&fprint);
							DBG_RETURN(connection);
						} else {
							restart_count++;
		...
		case USE_MASTER:
		{
			...
			/* LOCK on context ??? */
			switch (zend_hash_find(&filter->master_context, fprint.c, fprint.len /*\0 counted*/, (void **) &context_pos)) {
				case SUCCESS:
					connection = context_pos? *context_pos : NULL;
					smart_str_free(&fprint);
					if (!connection) {
...
restart_master:
					if (restart_count < zend_llist_count(l)) {
						...
						if (connection) {
							...
							zend_hash_update(&filter->master_context, fprint.c, fprint.len /*\0 counted*/, &connection, sizeof(MYSQLND *), NULL);
						} else {
							restart_count++;
							goto restart_master;					
						}
